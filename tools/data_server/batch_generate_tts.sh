...
# -------- config --------
VENV_DIR=".venv-data-server"
PYTHON_BIN="python3"
PY_SCRIPT="tools/data_server/python/tts_generate.py"
REQ_FILE="tools/data_server/requirements-tts.txt"
ENV_FILE=".env"                      # optional
# ------------------------

# Forward all args to python script, e.g. ./batch_generate_tts.sh sentences --ids "1-10" --lang fr
ARGS=("$@")
...
echo "▶ Running: python -m tools.data_server.python.tts_generate ${ARGS[*]:-}"
python -m tools.data_server.python.tts_generate "${ARGS[@]}"
