...
export class EditorStore {
   ...
   * sentenceAudioByKey:
   * - Key: "fr:123" / "de:123"
   * - Value: has_audio_(lang) from sentences.csv
   */
  readonly sentenceAudioByKey = computed(() => {
    const map = new Map<string, boolean>();
    for (const sentence of this.sentences() as any[]) {
      map.set(`fr:${sentence.id}`, !!sentence.has_audio_fr);
      map.set(`de:${sentence.id}`, !!sentence.has_audio_de);
    }
    return map;
  });
   ...
   * termAudioByKey:
   * - Key: "fr:123" / "de:123"
   * - Value: has_audio from terms.csv
   */
  readonly termAudioByKey = computed(() => {
    const map = new Map<string, boolean>();
    for (const term of this.terms() as any[]) {
      map.set(`${term.lang}:${term.id}`, !!term.has_audio);
    }
    return map;
  });

  hasSentenceAudio(lang: AudioLang, sentenceId: number): boolean {
    return this.sentenceAudioByKey().get(`${lang}:${sentenceId}`) ?? false;
  }

  hasTermAudio(lang: AudioLang, termId: number): boolean {
    return this.termAudioByKey().get(`${lang}:${termId}`) ?? false;
  }

  markSentenceHasAudio(lang: AudioLang, sentenceId: number, hasAudio: boolean) {
    this.sentences.set(
      (this.sentences() as any[]).map((sentence) => {
        if (sentence.id !== sentenceId) {
          return sentence;
        }

        if (lang === 'fr') {
          return { ...sentence, has_audio_fr: hasAudio };
        }

        return { ...sentence, has_audio_de: hasAudio };
      }),
    );
  }

  markTermHasAudio(lang: AudioLang, termId: number, hasAudio: boolean) {
    this.terms.set(
      (this.terms() as any[]).map((term) => {
        if (term.id !== termId || term.lang !== lang) {
          return term;
        }
        return { ...term, has_audio: hasAudio };
      }),
    );
  }
  ...
  private async loadFromTTSServer(): Promise<void> {
    ...
    // IMPORTANT: no audio existence checks here; CSV flags are the truth.
  }
  ...
  private parseLinks(csv: string): TermLinkRow[] {
    ...
    for (const row of csvRows) {
      ...
      parsedLinks.push({
        fr_id: frenchId,
        de_id: germanId,
        priority: toNum(row['priority']) ?? undefined,
      });
    }
  ...
  updateTerm(id: number, patch: Partial<TermRow>) {
    this.terms.set(this.terms().map((term) => (term.id === id ? { ...term, ...patch } : term)));
  }
  ...
  addLink(frenchId: number, germanId: number, priority?: number) {
    ...
    this.links.set(
      [...this.links(), { fr_id: frenchId, de_id: germanId, priority }]
        .sort((a, b) => (a.fr_id - b.fr_id) || (a.de_id - b.de_id)),
    );
  }
  ...
  /**
   * Call your TTS service elsewhere; on success call this.
   * Keeping this method as a convenient central place.
   */
  markSentenceAudioGenerated(lang: AudioLang, sentenceId: number) {
    this.markSentenceHasAudio(lang, sentenceId, true);
  }

  markTermAudioGenerated(lang: AudioLang, termId: number) {
    this.markTermHasAudio(lang, termId, true);
  }
